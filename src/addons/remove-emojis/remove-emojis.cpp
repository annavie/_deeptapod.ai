#include <iostream>
#include <string>
#include <regex>
#include <node.h>
#include <v8.h>

std::string removeEmojis(const std::string& text) {
    std::regex emojiRegex(R"([\u0080-\uFFFF])");
    return std::regex_replace(text, emojiRegex, "");
}

void RemoveEmojis(const v8::FunctionCallbackInfo<v8::Value>& args) {
    v8::Isolate* isolate = args.GetIsolate();

    if (args.Length() < 1 || !args[0]->IsString()) {
        isolate->ThrowException(v8::Exception::TypeError(
            v8::String::NewFromUtf8(isolate, "Argument must be a string").ToLocalChecked()));
        return;
    }

    v8::String::Utf8Value text(isolate, args[0].As<v8::String>());

    std::string result = removeEmojis(*text);

    args.GetReturnValue().Set(v8::String::NewFromUtf8(isolate, result.c_str()).ToLocalChecked());
}

void Initialize(v8::Local<v8::Object> exports) {
    NODE_SET_METHOD(exports, "removeEmojis", RemoveEmojis);
}

NODE_MODULE(NODE_GYP_MODULE_NAME, Initialize)
